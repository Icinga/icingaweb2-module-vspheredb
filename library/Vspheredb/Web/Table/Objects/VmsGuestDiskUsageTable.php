    ...
    protected function initialize(): void
    {
        $this->addAvailableColumns([
            ...
            $this->createColumn('free_space', $this->translate('Free Space'), 'vdu.free_space')
                ->setRenderer(fn($row) => Format::bytes($row->free_space)),

            $this->createColumn('capacity', $this->translate('Capacity'), 'vdu.capacity')
                ->setRenderer(fn($row) => Format::bytes($row->capacity)),

            $this->createColumn('usage', $this->translate('Usage'), [
                'free_space' => 'vdu.free_space',
                'capacity'   => 'vdu.capacity'
            ])
                ->setRenderer(fn($row) => new SimpleUsageBar($row->capacity - $row->free_space, $row->capacity, sprintf(
                    '%s free out of %s (%.2F %%)',
                    $row->free_space,
                    $row->capacity,
                    $row->free_space / $row->capacity * 100
                )))
                ->setSortExpression('1 - (vdu.free_space / vdu.capacity)')
                ->setDefaultSortDirection('DESC')
        ]);

        if ($this->withHistory) {
            $this->addAvailableColumn(
                $this->createColumn('history', $this->translate('History'), ['object_name' => 'o.object_name'])
